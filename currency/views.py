...
def check_user_currency(user_id,currency_code,currency_quantity):
    ...
    try:
        ...
        response['message'] = 'Something Went Wrong in check_user_currency'
        print(e," ERROR IN check_user_currency --line number of error {}".format(sys.exc_info()[-1].tb_lineno))    

    return response

def create_update_user_currency(user_id,add_currency_code,add_currency_quantity,reduce_currency_code,reduce_currency_quantity):
    status = False
    try:
        with transaction.atomic():
            user_wallet_obj = wallet_model.UserWallet.objects.filter(user_id=user_id,currency_code=add_currency_code).first()
            if user_wallet_obj:
                current_quantity = float(user_wallet_obj.currency_quantity)
                total_quantity = add_currency_quantity + float(current_quantity)
                user_wallet_obj.currency_quantity = total_quantity
                user_wallet_obj.save()
            else:
                wallet_model.UserWallet.objects.create(user_id=user_id,currency_code=add_currency_code,currency_quantity=add_currency_quantity)
            user_wallet_reduce = wallet_model.UserWallet.objects.filter(user_id=user_id,currency_code=reduce_currency_code).first()
            if user_wallet_reduce:
                quantity = float(user_wallet_reduce.currency_quantity)
                print(quantity - float(reduce_currency_quantity))
                total_quantity = quantity - float(reduce_currency_quantity)
        ...
        status = False
        print(e," ERROR IN create_update_user_currency --line number of error {}".format(sys.exc_info()[-1].tb_lineno))    
        ...
        if request.method == "POST":
            ...
            if decode_data:
                ...
                if verify_currency_code(from_currency) and verify_currency_code(to_currency):                    
                    quantity_validation = check_user_currency(user_id,from_currency,from_currency_quantity)
                    if quantity_validation['valid'] == True:
                        ...
                        converion_request = requests.get(conversion_url)
                        currency_data = converion_request.json()
                        from_currency_price = currency_data[from_currency+"_"+to_currency]
                        total_value_after_conversion = round(float(from_currency_quantity) * from_currency_price,2)
                        conversion_status = create_update_user_currency(user_id,to_currency,total_value_after_conversion,from_currency,from_currency_quantity)
